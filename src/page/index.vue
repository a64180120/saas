<template>
    <div class="home" style="background:#f4f4f4;">
        ...
        <!--轮播图-->
        <template v-if="menuMeesage.rotatechart&&menuMeesage.rotatechart.isshow">
            ...
            <div class="Broadcast_box" style="animation-delay:0.5s;">
                <div style="width: 1178px;margin-left: 11px; margin-top:4px; position: relative;">
                    ...
                    <el-carousel height="552px">
                        <el-carousel-item v-for="(item,index) in imgObj['轮播图']" :key="index">
        ...
        <div style="height: 80px" v-if="!menuMeesage.labfinadmin||!menuMeesage.datareportpla||!menuMeesage.labfinadmin.isshow||!menuMeesage.datareportpla.isshow"></div>
        <!--湖南注释-->
        <div class="Central_navigation"  v-if="(menuMeesage.labfinadmin&&menuMeesage.labfinadmin.isshow)||(menuMeesage.datareportpla&&menuMeesage.datareportpla.isshow)">
            <!--工会财务管理-->
            <template v-if="menuMeesage.labfinadmin&&menuMeesage.labfinadmin.isshow" >
                <div @click.stop="NavTo('finance')" class="Finance" :class="[menuMeesage.datareportpla&&menuMeesage.datareportpla.isshow ? '' : 'Finance-with']">
                    <div :class="[menuMeesage.datareportpla&&menuMeesage.datareportpla.isshow ? '' : 'Finance-center']">
                        <img src="@/assets/img/cai.png"/>
                        <h1>工会财务管理</h1>
                    </div>
                </div>
            </template>
            <!--数据直报平台-->
            <template v-if="menuMeesage.datareportpla&&menuMeesage.datareportpla.isshow">

                <div @click.stop="NavTo('report')"  :class="[menuMeesage.labfinadmin&&menuMeesage.labfinadmin.isshow? '' : 'data-with']" class="data">
                    <div :class="[menuMeesage.labfinadmin&&menuMeesage.labfinadmin.isshow ? '' : 'Finance-center']">
        ...
        <!--工会新闻-->
        <template v-if="menuMeesage.inddynam&&menuMeesage.inddynam.isshow">
            <div id="Journalism" style="margin-top:30px;" class="Journalism">
                <div class="biao">
                    <span style="min-width: 70px">{{menuMeesage.inddynam.title}}</span>
                    <img src="@/assets/img/b1-1.png" style="width:100%; margin-top:12px;"/>
                    <p class="g" style="cursor: pointer;" @click.stop="openNewsList(menuMeesage.inddynam.phid,menuMeesage.inddynam.title)">更多</p>
                </div>
                <div v-if="newsObj[menuMeesage.inddynam.phid]&&newsObj[menuMeesage.inddynam.phid].length>0">
                    <div class="Journalism_left"  @click.stop="openUrl(newsObj[menuMeesage.inddynam.phid][0],menuMeesage.inddynam.title)">

                        <div class="img_box">
                            <img :src="picUrl + newsObj[menuMeesage.inddynam.phid][0].Picpath" v-if="newsObj[menuMeesage.inddynam.phid][0].Picpath" style="width: 100%;height: 100%">
                            <!--<img src="@/assets/img/img1.png"/>-->
                        </div>
                        <div class="txt_box" >
                            <h1>{{newsObj[menuMeesage.inddynam.phid][0].Title}}</h1>
                            ...
                            <div v-html="newsObj[menuMeesage.inddynam.phid][0].Content">
                            </div>
                        </div>
                        <!-- <div class="txt_box1">
                             <p>一、凡享受企业所得税优惠政策的企业，均应按照23号公告执行。
                                 ...
                                 　　 三、本公告适用于2017年度汇算清缴及以后年度企业所得税优惠事项办理工作。《浙江省国家税务局 浙江省地方税务局关于企业所得税优惠事项办理问题的公告》（浙江省国家税务局
                                 浙江省地方税务局公告2015年第10号）以及《浙江省国家税务局关于加强企业所得税优惠备案管理工作的通知》（浙国税发〔2017〕60号）同时废止。</p>
                         </div>-->
                    </div>
                    <div class="Journalism_right">
                        <ul>
                            <li v-for="(item,index) of newsObj[menuMeesage.inddynam.phid]" v-if="index>0" :key="index" @click.stop="openUrl(item,menuMeesage.inddynam.title)">
                                <div>
                                    <i class="list-dot"></i>
                ...
                <div v-else style="text-align: center;line-height: 220px;">
                    当前新闻类别并未发布新闻
                </div>
            </div>
        ...
        <!--基层动态-->
        <template v-if="menuMeesage.productplan&&menuMeesage.productplan.isshow">
            <div id="Broadcast" class="Broadcast">
                <div
                    style="float:left;width:70px;background: #0183fd;color: #fff;font-size: 25px;padding: 5px 18px;text-align:center;margin: 20px 0 20px 20px;">
                    {{menuMeesage.productplan.title}}
                </div>
                <div id="div1">
                    <ul id="moveul" v-if="newsObj[menuMeesage.productplan.phid]&&newsObj[menuMeesage.productplan.phid].length>0">
                        <li v-for="(item,index) of newsObj[menuMeesage.productplan.phid]" :key="index" @click.stop="openUrl(item,menuMeesage.productplan.title)">
                            <img :src="picUrl + item.Picpath"/>
                            <div class="newsTitle">{{item.Title}}</div>
                        </li>
                    </ul>
                    <p v-else style="margin-top: 50px;text-align: center">未获取到{{menuMeesage.productplan.title}}</p>
                </div>
            </div>
        </template>
        <!--政策制度-->
        <template>
            <div id="policy" class="policy" style="position: relative">
                <div v-if="menuMeesage.policysystem&&menuMeesage.policysystem.isshow" :class="[menuMeesage.unionwindow&&menuMeesage.unionwindow.isshow?'policy-with2':'policy-with1']">
                    <div style="background-color: #0183FD;width: 100%;height: 33px">
                        <div class="b">
                    ...
                    <div class="policy_right">
                        <ul>
                            <li v-for="(item,index ) of newsInfo" :key="index" @click.stop="openUrl(item,menuMeesage.policysystem.title)">
                                <div>
                                    <i class="list-dot"></i>
                                    <p class="txt" style='cursor: pointer;'>{{item.Title}}</p>
                                    <span class="r">{{item.PublishTime | formatDate('YYYY-MM-DD')}}</span>
                    ...
                    <div class="switch">
                        <div style="float: right">
                            <img src="@/assets/img/j1.png" style="margin-right: 0px;margin-top: 14px;"/>
                            <p :class="loading?'changenews-true':'changenews-false' " @click.stop="changenewsList" style="margin-right: 10px">换一批</p>
                            <p style="margin-right: 8px;cursor: pointer;" @click.stop="openNewsList('291190117000002',menuMeesage.policysystem.title)">更多</p>
                        </div>
                    </div>
                </div>
                <div class="policy_left" :class="[menuMeesage.policysystem&&menuMeesage.policysystem.isshow?'policy-with2':'policy-with1']" style="position:absolute;top: 20px;right: 15px">
                    <!--公会之窗-->
                    <template v-if="menuMeesage.unionwindow&&menuMeesage.unionwindow.isshow">
                        <div class="chuang">

                        </div>
                        <el-carousel indicator-position="none" height="400px" arrow="never">
                            <el-carousel-item v-for="(item,index) in bottomPicList" :key="index">
                                <div class="newsTitle" style="top: 360px;height: 40px;line-height: 40px;"><span style="float: left;margin-left: 10px"><span style="color: red;font-size: 20px">{{index+1}}</span>{{'/'+bottomPicList.length}}</span>{{item.Title}}</div>
                                <a :href="item.Linkaddress?item.Linkaddress:'javascript:void(0)'"><img
                                    :src="picUrl+item.Picpath" alt="" style="width: 100%;height: 100%;margin-top: -40px;"></a>-
                            </el-carousel-item>
                            <!--                    <el-carousel-item>-->
        ...
        <!--工会新闻-->
        <template v-for="mnueItem in menuMeesage">
            <template v-if="mnueItem.sign==3&&mnueItem.isshow&&mnueItem.phid!='291190117000002'&&mnueItem.phid!='291190117000001'&&mnueItem.phid!='291190117000003'">
                <div  style="margin-top:30px;" class="Journalism">
                    <div class="biao">
                        <span style="min-width: 70px">{{mnueItem.title}}</span>
                        <img src="@/assets/img/b1-1.png" style="width:100%; margin-top:12px;"/>
                        <p class="g" style="cursor: pointer;" @click.stop="openNewsList(mnueItem.phid,mnueItem.title)">更多</p>
                    </div>
                    <div v-if="newsObj[mnueItem.phid]&&newsObj[mnueItem.phid].length>0">
                        <div class="Journalism_left"  @click.stop="openUrl(newsObj[mnueItem.phid][0],mnueItem.title)">

                            <div class="img_box">
                                <img :src="picUrl + newsObj[mnueItem.phid][0].Picpath" v-if="newsObj[mnueItem.phid][0].Picpath" style="width: 100%;height: 100%">
                                <!--<img src="@/assets/img/img1.png"/>-->
                            </div>
                            <div class="txt_box" >
                                <h1>{{newsObj[mnueItem.phid][0].Title}}</h1>
                                <!--<p>为进一步落实企业所得税优惠政策，统一规范企业所得税优惠事项的办理，根据《国家税务总局关于发布修订后的&lt;企业所得税优惠政策事项办理办法&gt;的公告》（国家税务总局公告2018年第23号，以下简称23号公告）的规定，结合我省实际，现就企业所得税优惠事项办理有关问题公告如下：</p>-->
                                <div v-html="newsObj[mnueItem.phid][0].Content">
                                </div>
                            </div>
                            <!-- <div class="txt_box1">
                                 <p>一、凡享受企业所得税优惠政策的企业，均应按照23号公告执行。
                        ...
                        <div class="Journalism_right">
                            <ul>
                                <li v-for="(item,index) of newsObj[mnueItem.phid]" v-if="index>0" :key="index" @click.stop="openUrl(item,mnueItem.title)">
                                    <div>
                                        <i class="list-dot"></i>
                                        <p class="txt" style='cursor: pointer;'>{{item.Title}}</p>
                                        <span class="r">{{item.PublishTime | formatDate('YYYY-MM-DD')}}</span>
        ...
        <template v-if="true">
            <div id="bottom_box" class="bottom_box">
                <!--联系方式-->
                <template v-if="menuMeesage.linkmode&&menuMeesage.linkmode.isshow">
                ...
                <!--在线留言-->
                <template v-if="menuMeesage.onlinemess&&menuMeesage.onlinemess.isshow">
        ...
        <div class="bottom_box1" :class="[]">
            <div style="width:1132px;  margin:auto auto 0;">
                <!--湖南注释-->
                <template v-if="menuMeesage.serviceagree&& menuMeesage.disclaimer&&menuMeesage.aboutnewgrade&&menuMeesage.aboutzhengyun&&menuMeesage.qqorweixin
                &&menuMeesage.serviceagree.isshow&&menuMeesage.disclaimer.isshow&&menuMeesage.aboutnewgrade.isshow&&menuMeesage.aboutzhengyun.isshow&&menuMeesage.qqorweixin.isshow" >
                    <!--左边内容-->
                    <template>
                        <div style="width:566px; height:220px; float:left;" :class="menuMeesage.qqorweixin&&menuMeesage.qqorweixin.isshow?'':'no-QQorweixin'">
                            <div class="zi" style="margin-top:70px;">
                                <!--服务协议-->
                                <template v-if="menuMeesage.serviceagree&&menuMeesage.serviceagree.isshow">
                                    <h1 style="margin-left: 29px;cursor: pointer;" @click="serviceVisible = true">服务协议</h1>
                                </template>
                                <p v-if="menuMeesage.disclaimer&&menuMeesage.disclaimer.isshow">|</p>
                                <!--免责声明-->
                                <template v-if="menuMeesage.disclaimer&&menuMeesage.disclaimer.isshow">
                                    <h1 style="cursor: pointer;" @click="operateVisible = true">免责声明</h1>
                                </template>
                            </div>
                            <div class="zi" style="margin-top: -2px;margin-left:87px;">
                                <!--关于新中大科技-->
                                <template v-if="menuMeesage.aboutnewgrade&&menuMeesage.aboutnewgrade.isshow">
                                    <a style="font-size: 20px;color: #fff;" href="http://www.newgrand.cn">关于新中大科技</a>
                                </template>
                                <!--关于政云数据-->
                                <template v-if="menuMeesage.aboutzhengyun&&menuMeesage.aboutzhengyun.isshow">
                    ...
                    <!--右边内容-->
                    <!--微信-->
                    <template v-if="menuMeesage.qqorweixin&&menuMeesage.qqorweixin.isshow">
                        <div style=" width:566px; height:220px; float:right;" :class="menuMeesage.serviceagree.isshow==false&&menuMeesage.disclaimer.isshow ==false&&menuMeesage.aboutnewgrade.isshow==false&&menuMeesage.aboutzhengyun.isshow==false?'no-disclaimer':''">
                ...
                <!--版权-->
                <template v-if="menuMeesage.copyright&&menuMeesage.copyright.isshow">
                    <div class="cc">
        ...
        <el-dialog title="修改密码" :visible.sync="dialog.editPaw.show" custom-class="editPawDialog">
            <el-form :model="editPaw" :rules="editPawRules" ref="editPaw" label-width="100px">
                ...
                <el-form-item label="确认新密码" prop="confirmNewPaw">
                    <el-input type="password" key="inpConfirmNewPaw" v-model="editPaw.confirmNewPaw"
            ...
            <el-dialog title="服务协议" :visible.sync="serviceVisible">
                <div class="fwxy">
                    ...
                    <p>
                        ...
                        您在使用GHC工会财务云平台提供的各项服务之前，应仔细阅读本用户协议。
                        您在注册程序过程中点击“同意条款，立即注册”按钮即表示您与GHC工会财务云平台达成协议，完全接受本服务条款项下的全部条款。<br>
                    ...
                    <ul>
                        ...
                        <li>
                            2.用户隐私信息是指涉及用户集体或个人身份或个人隐私的信息，比如，用户真实姓名、身份证号、手机号码、地址等。非个人隐私信息是指用户对本服务的操作状态以及使用习惯等明确且客观反映在GHC工会财务云平台服务器端的基本记录信息、个人隐私信息范围外的其它普通信息，以及用户同意公开的上述隐私信息。尊重用户个人隐私信息的私有性是GHC工会财务云平台的一贯制度，平台将采取技术措施和其他必要措施，确保用户及个人隐私信息安全，防止在本服务中收集的用户个人隐私信息泄露、毁损或丢失。在发生前述情形或者GHC工会财务云平台发现存在发生前述情形的可能时，将及时采取补救措施。平台未经用户同意不向任何第三方公开、
                            透露用户个人隐私信息。<br/>
                            但以下特定情形除外：
                            <ul>
                                <li>
                    ...
                    <ul>
                        <li>
                            1.发布或以其它方式传送含有下列内容之一的信息： 反对宪法所确定的基本原则的； 危害国家安全，泄露国家秘密，颠覆国家政权，破坏国家统一的； 损害国家荣誉和利益的；
                            煽动民族仇恨、民族歧视、破坏民族团结的； 破坏国家宗教政策，宣扬邪教和封建迷信的； 散布谣言，扰乱社会秩序，破坏社会稳定的； 散布淫秽、色情、赌博、暴力、凶杀、恐怖或者教唆犯罪的；
                            侮辱或者诽谤他人，侵害他人合法权利的； 含有虚假、诈骗、有害、胁迫、侵害他人隐私、骚扰、侵害、中伤、粗俗、猥亵、或其它道德上令人反感的内容；
                            含有中国法律、法规、规章、条例以及任何具有法律效力之规范所限制或禁止的其它内容的； 含有GHC工会财务云平台认为不适合在平台展示的内容；
                        </li>
                        <li>
                            2.以任何方式危害他人的合法权益；
                        </li>
                    ...
                    <p>
                        您已认可GHC工会财务云平台未对用户的使用行为进行全面控制，您使用任何内容时，包括依赖前述内容之正确性、完整性或实用性时，您同意将自行加以判断并承担所有风险，而不依赖于GHC工会财务云平台。但GHC工会财务云平台依其自行之考虑，可拒绝和删除经由本服务提供之违反本条款的或其它引起GHC工会财务云平台反感的任何内容。
                        您了解并同意，GHC工会财务云平台依据法律法规的要求，或基于诚信为了以下目的或在合理必要范围内，认定必须将内容加以保存或揭露时，得加以保存或揭露：</p>
                    <ul>
                        <li>
                            1.遵守法律程序；
                        </li>
        ...
        methods: {
            ...
            getNewsList(title,phid){
                ...
                this.GetNewsList(parames).then(res => {
                    ...
                    this.newsObj[phid]=res.List;
                    this.$forceUpdate(this.newsObj);
                    if(phid=='291190117000003'&&res.List.length>5){
                        this.cpMove(0)
                    }
                }).catch(error => {
                    console.log(error);
            ...
            openUrl(object,title) {
                ...
                this.$router.push({
                    ...
                    query: {
                        phid: object.PhId,
                        type: 'index',
                        typePhid: object.PhIdType,
                        menuMeesage:this.menuMeesage,
                        title:title
                    }
                });
            },
            ...
            //新闻 更多页面
            openNewsList(typeid,title) {
                this.$router.push({path: '/news/list', query: {typePhid: typeid,menuMeesage:this.menuMeesage,title:title}});
            },
            typeclick(object) {
                //alert(object);
                this.active = object;
            ...
            cpMove:function(po){
              setTimeout(()=>{
                  if(po<204){
                      po+=1;
                      document.getElementById('moveul').style=('margin-left:-'+po+'px');

                  }else{
                      po=0;
                      document.getElementById('moveul').style=('margin-left:0px');
                      let dom=document.getElementById('moveul').firstElementChild.cloneNode(true);
                      document.getElementById('moveul').removeChild(document.getElementById('moveul').firstElementChild);
                      document.getElementById('moveul').appendChild(dom);
                      dom=null;
                  }
                  this.cpMove(po);
              },20);
