  ...
  <div class="voucher"
    ...
    <div class="voucherHead">
      <ul class="flexPublic">
        ...
        <li class="flexPublic">
          <div @click.stop="picUploadShow"
               class="uploaderTitle"></div>
          <div class="fileContainer">
            <div>附单据&nbsp;</div>
            <div class="inputContainer"><input type="text"
                     :disabled="disabled"
                     v-model="PAttachment"></div>
            <div>&nbsp;张&nbsp;</div>
      ...
      <ul>
        <li>
          <ul class="flexPublic voucherContentItem">
            ...
            <li>
              <div>
                <div v-if="fatherData.Verify==1"
                     class="verifyPanel auditZhang">
                  <div :style="{lineHeight:fatherData.VerifyDate?'25px':'50px'}">已审核</div>
                  <div v-if="fatherData.VerifyDate"
                       style="font-size: 14px">{{fatherData.VerifyDate.slice(0,10)}}</div>
                </div>
          ...
          <ul class="flexPublic voucherContentItem">
            ...
            <li class="abstractContainer">
              <div class="inputContainer">
                <textarea class="inputFocus"
                          @keydown.tab.prevent="abstractTab(item,index,$event)"
                          @keydown.13.prevent="chooseAbstractOk($event,item,AbstractAContents[indAbs].AContent,index)"
                          @keydown.down.prevent="abstractDown(true)"
                          @keydown.up.prevent="abstractDown(false)"
                          :disabled="disabled"
                          :class="{chongHcss:AbstractCss=='冲红',gengZcss:AbstractCss=='更正'}"
                          v-model="item.Abstract"
                          @focus="showAddIcon(index,item,$event)"
                          @keyup.prevent="AbsSearch(item.Abstract,$event)"
                          @blur="defaultHandle(item,index)"
                          maxlength="50"></textarea>
              </div>
              <div v-if="addIcon[index].checked&&AbstractAContents.length>0"
                   class="abstractSearch">
                <ul>
                  <li @click.stop="chooseAbstractOk($event,item,abs.AContent,index)"
                      :title="abs.AContent"
                      v-for="(abs,indAbstract) of AbstractAContents "
                      :key="indAbstract"
                      :class="{keyActive:indAbs==indAbstract}">{{abs.AContent}}</li>
            ...
            <li @click.stop="handleKemuSel(index,item)"
                class="kemu">
              <div>
                <ul>
                  <li>
                  ...
                  <li v-show="item.SubjectCode&&(item.balance||item.balance===0)"><span>累计发生:</span><span>{{item.balance | Num}}</span></li>
              ...
              <searchSelect style="z-index:10"
                            ...
                            @item-click="subClickCheck"></searchSelect>
              <!--辅助核算选择-->
              <div @click.stop="1"
                ...
                <ul>
                  <li v-for="(assist,index2) of assistList"
                    ...
                    <div @click.stop="assistSelsShow(index2)"
                         class="assistSelCon">
                      <div>{{assistSels[index2].BaseName}}</div>
                      <ul v-show="assistSels[index2].checked">
                        <li @click.stop="AuxiliaryValOk(val,index2)"
                            v-for="(val,ind3) of assist.Children"
                            :title="val.BaseName"
                            :key="ind3"> {{val.BaseName}}</li>

                      </ul>
                      <div style="border:1px solid #999;border-top:0"
                           v-show="assistSels[index2].checked"
            ...
            <li @click="moneyInputShow(item,'daifang')"
              ...
              <span :class="{moneyInputShow:item.moneyInput.daifang}"
                ...
                <input class="inputFocus"
                       ...
                       @focus="moneyInputShow(item,'daifang')"
                       @blur="delayInputBlur($event,item,'daifang',index)"
                       placeholder="请输入金额"
                       @keydown.13.prevent="keyEnter($event)"
                       @keyup="clearNoNum($event)"
                       onafterpaste="this.value=this.value.replace(/e/g,'')">
        ...
        <ul>
          ...
          <li>
            ...
            <div style="padding-left:10px;border:1px solid #ccc">
              {{navList[subjectInfo.preSubject.KType-1]}}
            </div>
    ...
    <div v-if="addauxiliaryShow"
      ...
      <div class="addPage">
        ...
        <div>
          ...
          <span @click.stop="addauxiliaryFinish(true)"
                class="btn">保存</span>
...
export default {
  ...
  props: {
    ...
    'sideDate': String,
    'disabled': Boolean,
    'disabled2': { type: Boolean, default: true },//日期选择器剪切专用
    'balanceBool': { default: false },//是否拉取余额,引用模板时使用
    'AbstractList': { type: Array, default: () => [] },  //获取的摘要模板列表
    'vouSubjectlist': Array,//获取的科目列表*****************
  },
  data () {    return {
      removing: false,//附件删除中
      fatherData: '',
      fileList: [],
      PhId: '',
      PDate: '',
  ...
  created () {
    ...
    if ((!this.dataList.data.Mst) || typeof (this.dataList.data.Mst) == 'string') {//没有传参时初始化页面
      ...
      this.PKeepingPerson = this.curOrg.AccountPeople;
      this.PFinancePerson = this.curOrg.Treasurer;
      this.PCashier = this.curOrg.Cashier;
      this.getFreshVoucher();
    } else if (this.dataList.data.Mst && !this.dataList.data.Mst.PhId) {
  ...
  mounted () {
    //初始化科目列表
    this.subjectlist = this.vouSubjectlist;
  ...
  methods: {
    ...
    voucherData () {
      ...
      var dtls, fatherData;
      fatherData = JSON.parse(JSON.stringify(this.fatherData));
      if (!fatherData.PhId) {
        dtls = [];
      } else {
        dtls = fatherData.Dtls;
      }
      var account;
      var item;
      ...
      if (!this.PhId) {
        ...
        if (fatherData.PType != '结') {
          fatherData.PType = '记';
        }
        fatherData.OrgId = this.orgid;
        fatherData.OrgCode = this.orgcode;
        fatherData.PersistentState = 1;
      } else {
        fatherData.PersistentState = 2;
        fatherData.PhId = this.PhId;
      }
      fatherData.PDate = this.PDate;
      fatherData.PAttachment = this.PAttachment;

      if (!this.PMakePerson) {
        this.PMakePerson = this.username;
      }

      fatherData.PMakePerson = this.PMakePerson;
      fatherData.PFinancePerson = this.PFinancePerson;
      fatherData.PKeepingPerson = this.PKeepingPerson;
      fatherData.PCashier = this.PCashier;
      fatherData.PAuditor = this.PAuditor;
    ...
    //
    keyEnter (event) {
      var inputs = document.getElementsByClassName('inputFocus');
      for (let inp in inputs) {
        if (inputs[inp] == event.target && inp < inputs.length - 1) {
          inputs[parseInt(inp) + 1].focus();
        }
      }
    ...
    getFreshVoucher () {
      ...
      this.$axios.get('/PVoucherMst/GetVoucherList', { params: data })
        ...
        .catch(err => {
          loading1.close();
          this.$message({ showClose: true, message: '获取最新凭证时间失败了!', type: "error" });
        })
    },
    //获取父组件传参*********************************
    getVoucherData (data) {
      ...
      if (!data.PDate && data.PSource != '冲红' && data.PSource != '更正') {
        var year = this.sideDate.slice(0, 5);
        var month = this.sideDate.slice(5, 7);
        this.PDate = year + (month > 9 ? month : ('0' + month)) + '-01';
      } else {
    ...
    getAttachements (PhId) {
      ...
      this.$axios.get('PVoucherMst/GetAttachmentListByID', { params: data })
        ...
        .catch(err => {
          this.$message({ showClose: true, message: '获取附件失败了!', type: "error" });
        }
        )
    },
    // //获取摘要模板***移到父组件了
    ...
    // },
    //摘要确认键
    abstractTab (item, index, $event) {
      this.handleKemuSel(index, item);
      $event.target.blur();
    },
    //摘要上下选择
    abstractDown (bool) {
      var ul = document.querySelector('.abstractSearch').children[0];
      if (bool) {
        if (this.indAbs < this.AbstractAContents.length - 1)
          this.indAbs++;
        if (ul.children[this.indAbs].offsetTop > (ul.clientHeight + ul.scrollTop)) {
          ul.scrollTop = ul.scrollTop + 150;
        }
      } else {
        if (this.indAbs > 0)
          this.indAbs--;
        if (ul.children[this.indAbs].offsetTop < ul.scrollTop - 1) {  //-1px好像是边框
          ul.scrollTop = ul.scrollTop - 150;
        }
      }
    ...
    AbsSearch (val, $event) {
      ...
      this.AbstractAContents = [{ AContent: '' }];
      ...
      this.$axios.get("/PSubject/GetVoucherAuxiliaryBySubject", { params: data })
        .then(res => {
          ...
          if (res.length > 0) {
            ...
            this.assistList = res;
            //this.$set(this.assistItem[val.id],'checked',true);
            ...
            this.getBalance(val);
            this.assistList = [];
            var li = document.querySelector('.voucherContent>ul>li:nth-of-type(' + (parseInt(index) + 2) + ')');
            li.children[0].children[3].children[0].children[0].focus();
        ...
        .catch(err => { this.$message({ showClose: true, message: '获取辅助项失败了!', type: "error" }); loading1.close(); this.moneyInputMask = false; })

    },
    //显示下拉
    assistSelsShow (index2) {
      for (var ass in this.assistSels) {
        if (ass != index2) {
          this.assistSels[ass].checked = false;
        }
      }
    ...
    AuxiliaryValOk (val, index2) {
      ...
      data.checked = false;
      this.assistSels[index2] = data;
      this.$forceUpdate();
    },
    //增加辅助项的值***
    addAuxiliaryVal (assist, index) {
      this.addAuxiliaryValShow = true;
      this.addAuxiliaryValForm = {   //页面上的值
        EnabledMark: 0,
        BaseName: ''
      },
      ...
      this.$axios.post('/PVoucherAuxiliaryType/PostUpdateAuxiliary', this.addAuxiliaryValData)
        .then(res => {
          ...
          if (res.Status == 'success') {
            ...
            this.getAssist(this.addauxiliarySub, this.addauxiliarySub.id);
        ...
        .catch(err => {
          loading.close();
          this.$message({ showClose: true, message: '保存辅助项值失败了!', type: "error" });
        })
    },
    //增加科目下的辅助项
    addauxiliary (val) {
      ...
      this.$axios.get('PSubject/GetPSubjectLastList', { params: data1 })
        ...
        .catch(err => {
          loading2.close();
          this.$message({ showClose: true, message: '保存辅助项失败了!', type: "error" });
        })
    ...
    addAuxiliaryType () {
      if (!this.addAuxiliaryForm.BaseName) {
        ...
        this.$axios.post('/PVoucherAuxiliaryType/PostAddAuxiliarySingleType', data)
          ...
          .catch(err => {
            loading.close();
            this.$message({ showClose: true, message: '保存辅助项类型失败了!', type: "error" });
          })
      }
    },
    //新增辅助项类型保存完成*****
    addauxiliaryFinish (bool) {
      ...
      SubjectUpdate(vm, data2)
        ...
        .catch(err => {
          ...
          loading2.close();
          this.$message({ showClose: true, message: '新增辅助项失败了!', type: "error" });
        })
    },
    //辅助项选择完成********************
    assistOk (bool, item, index) {
      ...
      if (bool) {
        ...
        fzcode = '';
        if (item.DtlAccounts.assistItem.length) {
          item.DtlAccounts.assistItem.map(ass => {
            fzcode += ass.BaseCode + '_'
          })
        }
        fzcode = fzcode.replace(/_$/, '');
        msg.data = {
          KCode: item.SubjectCode,
          FZcode: fzcode
        }
        this.getBalance(msg);
    ...
    //判断科目506/404...湖南  + 结转模板选择自动带出余额
    subClickCheck (childMsg) {
      var reg = /^506|^404/;
      if (reg.test(childMsg.data.KCode)) {
        this.$emit('addZindex', true);
        this.confirm = {
          type: 'subCheck',
          message: childMsg.data.KCode + '科目不建议基层工会使用,是否继续?',
          data: JSON.stringify(childMsg),
          btn1: '是',
          btn2: '否',
          visible: true
        }
      } else {
        this.itemClick(childMsg);
      }
    ...
    itemClick (childMsg) {
      ...
      //控制科目方向的
      // if(childMsg.data.KCode[0]=='3'||childMsg.data.KCode[0]=='4'||childMsg.data.KCode[0]=='5'){
      //     this.voucherInfo[childMsg.id].money.KBalanceType=childMsg.data.KBalanceType;
      //     var children;
      //     console.log(document.querySelectorAll('.voucherContent>ul>li')[parseInt(childMsg.id)+1].children[0].children[3]);
      //     if(childMsg.data.KBalanceType==1){
      //         this.voucherInfo[childMsg.id].money.daifang='';
      //         children=document.querySelectorAll('.voucherContent>ul>li')[parseInt(childMsg.id)+1].children[0].children[4].children;
      //
      //     }else if(childMsg.data.KBalanceType==2){
      //         this.voucherInfo[childMsg.id].money.jiefang='';
      //         children=document.querySelectorAll('.voucherContent>ul>li')[parseInt(childMsg.id)+1].children[0].children[3].children;
      //     }
    ...
    //获取引用模板的科目余额****
    getModelBalance () {
      this.$emit('getBalance', false);
      var kcodes = '';
      let KcodesList = [];
      let fzcode = '';
      for (var item of this.voucherInfo) {
        fzcode = ''
        if (item.DtlAccounts.assistItem.length) { //辅助项
          item.DtlAccounts.assistItem.map(ass => {
            fzcode = fzcode + ass.AuxiliaryCode + '_';
          })
          fzcode = fzcode.replace(/_$/, '');
        ...
        if (item.SubjectCode) {
          kcodes = kcodes + (item.SubjectCode) + ','
        }

      }
      if (!KcodesList.length) {
        return;
      }

      kcodes = kcodes.replace(/,$/, '');
      var data = {
        Year: this.sideDateNew.slice(0, 4),
        OrgIds: this.orgid,
        Kcode: kcodes,
        KcodesList: KcodesList,
        IsStart: ''
      }

      const loading5 = this.$loading();
      //不会排序,按发送的数据顺序传回
      this.$axios.post('/PVoucherMst/postSubjectBalance2', data)
        .then(res => {
          loading5.close();
          if (res.Status == 'error') {
            this.messageCallback(res.Msg);
            ...
            for (var info in res.Record) {
              this.voucherInfo[info].balance = res.Record[info].balance;
            }
            if (this.fatherData.PType == '结' && this.fatherData.PSource == 2) {
              var newArr = [];
              var reg = /^3310/;
              var sum = 0;
              var tab = '';
              for (var v of this.voucherInfo) {
                //取出不是3310...科目的有余额的科目,把余额放到金额里
                if (v.balance && v.balance != 0 && !(reg.test(v.SubjectCode))) {
                  if (this.fatherData.Description == 1) {
                    v.money.jiefang = v.balance;
                  } else if (this.fatherData.Description == 2) {
                    v.money.daifang = v.balance;
                  }
                  sum = parseFloat(sum) + parseFloat(v.balance)
                  newArr.push(v);
                }
                //把3310...科目取出来,
                if (reg.test(v.SubjectCode)) {
                  tab = v;
              ...
              //把其他科目余额总和给3310科目
              if (this.fatherData.Description == 1) {
                newArr[0].money.daifang = sum;
              } else if (this.fatherData.Description == 2) {
                newArr[0].money.jiefang = sum;
              }
              this.voucherInfo = newArr;
            }
        ...
        .catch(err => {
          loading5.close();
          console.log(err)
          this.$message({ showClose: true, message: '获取模板的科目余额失败了!', type: "error" });
    ...
    getBalance (Msg) {
      ...
      var data = {
        Year: this.sideDateNew.slice(0, 4),
        OrgIds: this.orgid,
        Kcode: Msg.data.KCode,
        FZcode: Msg.data.FZcode,
        IsStart: ''
      }
      const loading5 = this.$loading();
      this.$axios.get('/PVoucherMst/GetSubjectBalance', { params: data })
        .then(res => {
          ...
          if (this.fatherData.PType == '结') {
            var reg = /^3310/;
            var v = this.voucherInfo[Msg.id];
            var sumD = 0, sumJ = 0;
            if (reg.test(v.SubjectCode)) {
              for (var vou of this.voucherInfo) {
                sumJ = parseFloat(sumJ) + parseFloat(vou.money.jiefang);
                sumD = parseFloat(sumD) + parseFloat(vou.money.daifang);
              }
              if (this.fatherData.Description == 1) {
                v.money.jiefang = sumD;
              } else if (this.fatherData.Description == 2) {
                v.money.daifang = sumJ;
              }
            }
            //取出不是3310...科目的有余额的科目,把余额放到金额里
            if (v.balance && v.balance != 0 && !(reg.test(v.SubjectCode))) {
        ...
        .catch(err => {
          console.log(err)
          this.$message({ showClose: true, message: '获取科目余额失败了!', type: "error" });
          loading5.close();
        })
    },
    //选择框显示********************
    handleKemuSel (index, item) {
      for (var kemu in this.kemuSel) {
        this.assistItem[kemu].checked = false;
        this.kemuSel[kemu].checked = false;
      }
      ...
      //点击科目带出金额摘要
      if (!item.Abstract) {
        item.Abstract = this.defaultData.Abstract.replace(/\s+$/, ''); //摘要为空自动添加上一个摘要
      }
    ...
    delayInputBlur ($event, item, value, index) {
      if (!parseFloat(item.money[value])) {
        item.money[value] = '';
      }
      var input;
      var children;
      ...
      //判断结转模板方向
      var reg = /^3310/;
      if ((!reg.test(item.SubjectCode)) && this.fatherData.PType == '结') {
        if (this.fatherData.Description == '1') { //收入结转
          if (item.money.jiefang < 0 || item.money.jiefang > item.balance) {
            item.money.jiefang = '';
            item.moneyInput[value] = false;
            this.moneyInputMask = false;
            this.moneyTurn('', $event.target.parentNode.parentNode.children);
            this.countJie++;
            this.countDai++;
            this.messageCallback('结转金额必须大于等于0,小于等于科目余额');
            return;
          }
        } else if (this.fatherData.Description == '2') {
          if (item.money.daifang < 0 || item.money.daifang > item.balance) {
            item.money.daifang = '';
            item.moneyInput[value] = false;
            this.moneyInputMask = false;
            this.moneyTurn('', $event.target.parentNode.parentNode.children);
    ...
    inputBlur ($event, item, value, index) {
      ...
      //下一行非结转凭证 有摘要或者科目自动添加金额
      if (this.fatherData.PType != '结' && ($event.target && nextTr && (nextTr.Abstract || nextTr.SubjectCode) && (!nextTr.money.jiefang) && (!nextTr.money.daifang))) {
    ...
    moneyInputShow (item, val) {//金额输入框展示**********************
      //科目方向与输入方向不一致
      // if((item.money.KBalanceType==1&&val=='daifang')||(item.money.KBalanceType==2&&val=='jiefang')){
      //     return;
      // }
    ...
    //从摘要模板列表选择摘要
    chooseAbstractOk (event, item, val, index) {
      if (!val) {
        this.handleKemuSel(index, item);
        this.defaultData.Abstract = item.Abstract.replace(/\s+$/, '');
        return;
      }
      item.Abstract = val;
      this.defaultData.Abstract = val.replace(/\s+$/, '');;
      this.handleKemuSel(index, item);  //摘要选择完成切换科目选择
    },
    defaultHandle (val, index) {  //保存上一个摘要
      var vm = this;
      vm.defaultData.Abstract = val.Abstract;
      vm.addIcon[index].checked = false;
      setTimeout(function () {
        vm.$forceUpdate();
      }, 200)
      //this.$forceUpdate();
    ...
    addNewSubject () {  //获取所有科目,和辅助项
      ...
      this.$axios.get('PSubject/GetPSubjectLastList', { params: data1 })
        ...
        .catch(err => {
          loading2.close();
          this.$message({ showClose: true, message: '获取科目和辅助项失败了!', type: "error" });
        })
    },
    //接收addsub选中的值**
    subAddData (data) {
    ...
    addFinish (bool) {
      ...
      for (var t in this.addDataAux) {
        if (this.subjectInfo.AuxiliaryTypes[t]) {
          ...
          if (auxiBreak && (this.addDataAux[t].BaseName == '往来个人' || this.addDataAux[t].BaseName == '往来单位')) {

            this.messageCallback('辅助项"往来个人"与"往来单位"不能同时选择!');
            return;
          }
          if (this.addDataAux[t].BaseName == '往来个人' || this.addDataAux[t].BaseName == '往来单位') {
            auxiBreak = true;
          }
    ...
    confirmNo (type, data) {
      ...
      switch (type) {
        case 'subCheck':
          data = JSON.parse(data);
          this.$emit('addZindex', false);
          this.kemuSel[data.id].checked = false;
          this.$forceUpdate();
          break;
      }
  ...
  watch: {
    ...
    picVisible (val) {
      this.$emit('addZindex', val);
    },
    fatherData (val) {
      var data = val;
      var list = [];

      this.subjectlist = JSON.parse(JSON.stringify(this.vouSubjectlist));
    ...
    imglist (val) {
      if (val.length > this.PAttachment) {
        this.PAttachment = val.length;
      }
    }
