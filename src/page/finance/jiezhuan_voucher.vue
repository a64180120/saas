            ...
            <li class="abstractContainer">
              <div class="inputContainer">
                <textarea class="inputFocus"
                          @keydown.tab.prevent="abstractTab(index,$event)"
            ...
            <li @click.stop="handleKemuSel(index,item)"
              ...
              <div>
                <ul>
                  ...
                  <li v-show="item.SubjectCode&&(item.balance||item.balance===0)"><span>余额:</span><span>{{item.balance | Num}}</span></li>
...
export default {
  ...
  methods: {
      ...
      this.$axios.get("/PSubject/GetVoucherAuxiliaryBySubject", { params: data })
        .then(res => {
          ...
          if (res.length > 0) {
            ...
            this.assistList = [];
            var li = document.querySelector('.nextMonthCheck .voucherContent>ul>li:nth-of-type(' + (parseInt(index) + 2) + ')');
    ...
    initMoneyCss () {  //金额输入框样式初始化***************
      var li = document.querySelectorAll(".nextMonthCheck .voucherContent>ul>li");
      for (var i in this.voucherInfo) {
        ...
        var jie = document.querySelector(".nextMonthCheck .count>li:nth-child(2)");
        this.moneyTurn(this.jiefang, jie.children);
        var dai = document.querySelector(".nextMonthCheck .count>li:nth-child(3)");
