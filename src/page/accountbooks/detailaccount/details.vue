      ...
      <div class="manageContent">
        <div class="reportBox">
          <div class="unionState"
               style="width: 100%;height: 40px;min-width: 1260px;">

            <div style="width:100%;float: right"
                 id="elSelf">
              <div class="flexUl handle"
                ...
                <el-checkbox style="margin-left: 10px"
                             v-model="fz">显示辅助项</el-checkbox>
              </div>
              <el-date-picker v-model="dateSelect"
                              style="    height: 30px;    width: 225px;margin-left: 15px;
                                position: relative;
                                top: 1px;"
                              type="monthrange"
                              align="right"
              ...
              <ul class="flexUl handle">
                <li v-if="Roleauthorize.expenseAccount_export!=0"
                    @click="showSubSelect(0)">导出</li>
                <li v-if="Roleauthorize.expenseAccount_print!=0"
                    @click="showSubSelect(1)">打印</li>
              ...
              <ul class="flexUl handle"
                ...
                <div class="flexPublic handle">
                  ...
                  <div class="searcherBtn"
                       @click="selectBtn">搜索</div>
                  <div class="searcherBtn"
                       @click="showType='block'"
                       style="margin-left: 10px">高级</div>
                  <div class="searchPanel"
                       :style="{'display':showType}">
                    <div class="searchPanel_title">
                      <h1>高级查询</h1>
                      <i class="el-icon-close"
                         @click="showType='none'"></i>
                    </div>
                    <div class="searchPanel_content">
                      ...
                      <div>
                        <span>凭证号码</span>
                        <div class="flexPublic searchContentArea"
                             style="padding-left:14px;margin-right: 6px;">
                    ...
                    <div class="flexPublic searchPanel_bottom">
                      <div class="searchPanel_btn greybtn"
                           @click="clearPorp">重置</div>
                      <div class="searchPanel_btn bluebtn"
                           @click="searchDetail">搜索</div>
                    </div>
          ...
          <div class="flexPublic  formData">
            <div class="unionLists">
              <div class="unionListsTitle">
                ...
                <div class="el-input el-input--prefix"
                     style="text-align: center; padding: 3px 10px;height: 35px;overflow: hidden">
                  <div style="width: 158px;height: 26px;line-height: 26px;margin: auto">
                    <input type="text"
                           autocomplete="off"
                           placeholder="搜索科目编码/名称"
                           class="el-input__inner"
                           style="width: 158px;height: 26px;line-height: 26px;font-size: 10pt;"
                           @keyup.enter.up="searchCode">
                    <i class="el-input__icon el-icon-lx-search"
                       style="position: relative; left: -60px;z-index: 14; height: 26px;top: -28px;color: #dcdfe6;"></i>
            ...
            <div class="formData"
                 style="left: 270px;top: 0px;right: 0px;bottom: 0;width: auto;margin-top: 0;">
              <div class="tbHeader">
                <table>
                  <colgroup v-if="!fz">
                    <col width="12%" />
                ...
                <table ref="printFrom">
                  ...
                  <tbody>
                    <tr v-if="dataInfoMonth.Pdate!=undefined">
                      ...
                      <td class="right">
                        <!--{{dataInfoMonth.JSum| NumFormat}}-->
                      </td>
                      <td class="right"
                          :title="dataInfoMonth.DSum">
                        <!--{{dataInfoMonth.DSum| NumFormat}}-->
                      </td>
                      ...
                      <td v-if="fz">{{dataInfoMonth.AuxiliaryDel}}</td>
                    </tr>
                    <template v-for="(item,index) of dataInfo">
                      <tr v-if="item.Abstract=='科目初始化'"
                          :key="item.uid">
                        ...
                        <td v-if="fz">{{item.AuxiliaryDel}}</td>
                      </tr>
                      <tr v-else>
                        ...
                        <td class="center"><a @click="showvoucher"
                             :data-title="item.PhIdMst">{{item.Pno!='本月合计'&&item.Pno!='本年累计'?'记 -'+item.Pno:''}}</a></td>
                        <!--item.PType+-->
                    ...
                    <tr v-if="!busy">
                      <td class="center bolder"
                          colspan="7"
                          @click="getData(true)">加载更多</td>
                    </tr>
                  </tbody>
                </table>
              </div>
        ...
        <!--凭证组件弹窗-->
        <div class="voucherCover"
             v-if="voucherDataList.bool"
             style="z-index:999">
          <div class="voucherContent">
            <div class="voucherCover_title">凭证查看 <i @click="closeVoucher"
                 class="el-icon-close"></i></div>
            <div class="voucherHandle">
              <span v-if="Roleauthorize.expenseAccount_edit!=0"
                    v-show="!voucherAdd"
                    class="btn"
                    @click.stop="handle('keep')">保存</span>
              <span v-if="Roleauthorize.expenseAccount_edit!=0"
                    v-show="!voucherAdd"
                    class="btn"
                    @click.stop="handle('cancle')">取消</span>
              <span v-if="Roleauthorize.expenseAccount_edit!=0"
                    v-show="voucherAdd"
                    @click.stop="handle('update')"
                    class="btn">修改</span>
              <span v-if="Roleauthorize.expenseAccount_delete!=0"
                    v-show="voucherAdd"
                    @click.stop="handle('delete')"
                    class="btn">删除</span>
              <span v-if="Roleauthorize.expenseAccount_audit!=0"
                    v-show="voucherAdd"
                    @click.stop="handle('audit')"
                    class="btn">审核</span>
              <span v-if="Roleauthorize.expenseAccount_review!=0"
                    v-show="voucherAdd"
                    @click.stop="handle('unaudit')"
                    class="btn">反审核</span>
            </div>
            <voucherdetail :dataList="voucherDataList"
                           v-if="voucherAdd"
                           ref="voucher"></voucherdetail>
            <div v-else>
              <!--<div :class="{voucherDisabled:voucherAdd}"></div>-->

              <voucher :vouSubjectlist="vouSubjectlist"
        ...
        <!--Excel导出，科目选择-->
        <div class="cover"
             :style="{'display':(showExcelSelect?'block':'none'),'z-index':'999'}">
          <div class="coverContent"
               style="width: auto;height: auto;top: 20%;overflow-y: hidden">
            <div class="flexPublic">
              <p>科目选择</p>
              <i class="el-icon-close"
                 style="cursor: pointer;text-align:right"
                 @click="showExcelSelect=false"></i>
            </div>
            ...
            <div style="margin: 15px 15px 0 35px;text-align: left">
              <el-checkbox v-model="checked"
                           @change="checkedAll">全选</el-checkbox>
              <el-checkbox v-model="bnfs"
                           @change="dispart">只显示本年有发生或有余额的科目</el-checkbox>
            </div>
            <div v-if="postOrPrint==0"
                 style="margin-bottom: 15px;margin-left:35px;text-align: left">
              <el-checkbox v-model="postType">分表导出(是否分多个Excel导出)</el-checkbox>
            </div>
            <ul class="flexPublic handle"
                style="margin-bottom: 20px">

              <li style='margin:0 0 0px 20px;cursor: pointer'
                  @click="showExcelSelect=false">取消</li>
              <template v-if="postOrPrint==0">
                <li style='margin:0 0 0px 20px;cursor: pointer'
                    @click="postBalanceSheetExcel">确定</li>
              </template>
              <template v-else>
                <li style='margin:0 0 0px 20px;cursor: pointer'
                    @click="getDetailAccountAll">打印</li>
              </template>
          ...
          <div class="coverContent"
            ...
            <div id="printContentArea"
              ...
              <div class="timeSelect"
                   style="padding: 40px 40px 30px 30px">
                <template v-for="print in printDetailList">
                  <table style="margin-top:30px;page-break-before: always">
                    ...
                    <thead>
                      <tr>
                        <td :colspan="fz?8:7"
                            style="background-color: #fff;font-weight: bolder;font-size: 20px;border: 0;height: 30px">[{{ print.subName }}] 明细账</td>
                      </tr>
                      <tr>
                        <td :colspan="fz?8:7"
                    ...
                    <tbody>
                      ...
                      <template v-for="(item,index) of print">
                        ...
                        <tr v-else>
                          <td class="center">{{item.Abstract=='本月合计'||item.Abstract=='本年累计'?item.Pdate.split('-')[0]+'-'+item.Pdate.split('-')[1]:item.Pdate.slice(0,10).split(' ')[0]}}</td>
                          <td class="center"><a @click="showvoucher"
                               :data-title="item.PhIdMst">{{item.Pno!='本月合计'&&item.Pno!='本年累计'?item.PType+'-'+item.Pno:''}}</a></td>
                          <td :class="{'bolder':item.Abstract=='本月合计'||item.Abstract=='本年累计','align-center':true}">{{item.Abstract}}</td>
            ...
            <ul class=" handle"
                style="margin-bottom: 20px;margin-top: 20px"></ul>
          </div>
        </div>
      </div>
    </div>
...
*/
    ...
    dateChange: function (val) {
      ...
      // this.searchYear = parseInt(val[0].slice(0, 4));
      // this.getSubjectData();
    },
    ...
    dateChoose: function (val) {
      ...
      this.$set(this.dateSelect, 0, da1)
      this.$set(this.dateSelect, 1, da2)
    ...
    postBalanceSheetExcel: function () {
      ...
      for (let i = 0; i < checkedlist.length; i++) {
        codelist.push(checkedlist[i].KCode);
      }
      ...
      let url = "/PVoucherMst/PostDetailAccountExcel";
      if (!this.postType) {
        url = "/PVoucherMst/PostDetailAccountByOneExcel";
      }
    ...
    dispart: function () {
      ...
      if (this.bnfs) {
        ...
        this.$axios.get('PSubject/GetSubjectByVoucher', { params: data }).then(res => {
        ...
        this.subjectListsCheck = this.subjectLists;
      }
    ...
    getDetailAccountAll: function () {
      ...
      let url = "PVoucherMst/GetDetailAccountAll";
      axios.create(base).get(url, { params: param, headers: new_header }).then(res => {
        console.log(res);
        ...
        if (res.status != 200) {
          ...
          for (var i in data.Record) {
            data.Record[i]['subName'] = checkedlist[i].KCode + ' ' + checkedlist[i].KName;
          }
