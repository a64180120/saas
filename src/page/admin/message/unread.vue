<template>
    <div class="manageContent">
       <!-- <span v-if="unreadData.length>-1">{{unreadData.length>99?99:unreadData.length}}</span>-->
        <div class="reportBox">
            <div class="unionState" style="width: 100%;height: 40px;min-width: 1260px;">
                ...
                <div style="width:100%;float: right" id="elSelf" >
                    <ul class="flexUl handle">
                        <li @click="markRead">标为已读</li >
                        <li @click="markReadAll">全部已读</li >
                    ...
                    <ul  class="flexUl handle" style="line-height: 30px">
                        <div class="flexPublic handle">
                            ...
                            <div  class="searcherBtn" @click="getUnreadData">搜索</div>
                        </div>
                    </ul>
                </div>
            </div>
            <div class="formData" style="left:40px;top: 40px;right: 0px;bottom: 0;width: auto;margin-top: 0;">
                ...
                <div class="tbBody">
                    <table  ref="printFrom">
                        ...
                        <tbody>
                        <template v-if="unreadData.length>0">
                            <tr v-for="(item,index) in unreadData" :class="{'select':index==choosedIndex}" @click="choose(index)">
...
<script>
    ...
    export default {
        ...
        data(){
            return {
                inputKvalue:'',
                unreadDataList:[],//页面显示的清单
                unreadData:[],//未读消息清单
                showPrint:false,//显示隐藏打印区域
                choosedIndex:null,//选中的列
                saasMessage:{
                    visible:false,  //消息弹出框*******
                    message:'' //消息主体内容**************
                },
        ...
        methods:{
            ...
            markRead:function(){
                if(this.choosedIndex==null){
                    ...
                    //this.$message({showClose: true, message: '请先选中一行', type: 'error'});
                    return;
                }
                let data={
                    uid:this.userid,
                    value:this.unreadData[this.choosedIndex].PhId
                };
                let loading=this.$loading()
                this.$axios.post('SysMessage/PostIsFinish',data).then(res=>{
                    console.log(res);
                    this.saasMessage={
                        message:res.Msg,
                        visible:true
            ...
            markReadAll:function(){
                ...
                let loading=this.$loading()
                this.$axios.post('SysMessage/PostIsFinishAll',data).then(res=>{
                    this.saasMessage={
                        message:res.Msg,
                        visible:true
                    }
            ...
            choose:function(index){
                this.choosedIndex=this.choosedIndex==index?null:index;
            },
            //滚动事件
